            ...
            double inc, coder::array<double, 2U> &ccc)
{
    ...
    for (b_i = 0; b_i < i; b_i++) {
        ...
        coder::c_log(tmp_data);
        for (i2 = 0; i2 < 8; i2++) {
            maxval = 0.0;
            for (i3 = 0; i3 < 16; i3++) {
                maxval += dctcoef[i2 + (i3 << 3)] * tmp_data[i3];
            }
    ...
    ccc.set_size(tmp_size, input_sizes_idx_1 + sizes_idx_1);
    loop_ub = input_sizes_idx_1;
    for (i = 0; i < loop_ub; i++) {
        for (i1 = 0; i1 < tmp_size; i1++) {
            ccc[i1 + ccc.size(0) * i] = m[i1 + m.size(0) * i];
        }
    }
    loop_ub = sizes_idx_1;
    for (i = 0; i < loop_ub; i++) {
        for (i1 = 0; i1 < tmp_size; i1++) {
            ccc[i1 + ccc.size(0) * (i + input_sizes_idx_1)] =
                dtm[i1 + dtm.size(0) * i];
        }
    }
    ...
    loop_ub = ccc.size(1) - 1;
    for (i2 = 0; i2 <= loop_ub; i2++) {
        tmp_size = i1 - i;
        for (i3 = 0; i3 < tmp_size; i3++) {
            ccc[i3 + tmp_size * i2] = ccc[((i + i3) + ccc.size(0) * i2) + 1];
        }
    }
    ccc.set_size(i1 - i, loop_ub + 1);
}
